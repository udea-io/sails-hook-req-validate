...
module.exports = {
  types: {
    ...
    array: {
      method: (val) => { try { return _.isArray(_.isString(val) ? JSON.parse(val) : val); } catch (e) { return false; } },
      message: 'array',
    },
    ...
    json: {
      method: (val) => _.isPlainObject(val),
    ...
    object: {
      method: (val) => { try { const t = _.isString(val) ? JSON5.parse(val) : val; return _.isPlainObject(t); } catch (e) { return false; } },
      message: 'object',
    },
    ...
    nullable: {
      method: (val) => val === null || val === '',
      message: 'null or empty string',
    }
  },
  ...
  converters: {
    ...
    removespace: {
      converter: (val) => val.toString().replace(/\s/g, ''),
    },
    removelinebreak: {
      converter: (val) => val.toString().replace(/(\r\n\t|\n|\r\t)/gm, ''),
    },
