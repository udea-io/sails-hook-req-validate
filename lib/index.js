...
module.exports = function (rules, options, callback) {
  ...
  function errorBuilder(errors, errKeys) {
    let output = '';
    if (_.isString(errors)) { output = errors; } else if (_.isArray(errors)) { output = _.forEach(errors, (err) => { output += `${err.trim()} `; }); }
   ...
   */
    ...
    _.forEach(rules, (value, key) => {
      ...
      if (!isInParams(key)) {
        ...
        const validation = helper(options, key, value, param);
        if (validation.error) {
      ...
      // CASE 3b  ex) req.validate([{ 'myparam': 'string' }]);
        ...
        _.forEach(rule, (_value, _key) => {
          ...
          if (!isInParams(_key)) {
            ...
            const validation = helper(options, _key, _value, param);
            if (validation.error) {
              validation.error = _.isArray(validation.error) ? validation.error : [validation.error];
              _.forEach(validation.error, (err) => {
                errors.push({ // <-- ERROR
